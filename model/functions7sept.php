...
function getTeacher($id){
	global $conn;
	$sql="select t_name from teachers where t_id='$id'";
	$res = mysqli_query($conn,$sql);
	$row = mysqli_fetch_array($res);
	return $row['t_name'];
}
...
function getStudentName($id){
	global $conn;
	$sql="select student_name from students where ecode='$id'";
	$res = mysqli_query($conn,$sql);
	$row = mysqli_fetch_array($res);
...
function getTeacherVideo($id){
	global $conn;
	$sql="select vid_teacher,t_name from video v , teachers t where vid_id='$id' and vid_teacher=t.t_id";
	$res = mysqli_query($conn,$sql);
	$row = mysqli_fetch_array($res);
	$arr['teacherid']= $row['vid_teacher'];
	$arr['teachername']= $row['t_name'];
	return $arr;
}
...
function getWatchingVideos(& $emp_ecode){
	global $conn,$filelocation;
	$sql="select v.id,title,vthumb,watchtime from courses_videos v,videotrack t where t.video=v.id AND t.student='$emp_ecode' order by t.id desc limit 10";
	$res = mysqli_query($conn,$sql);
	$arr=array();
	$i=0;
	while($row = mysqli_fetch_array($res)){
...
function getLiveSessions(){
	...
	$sql="select vid_id,vtitle,vdesc,vthumb,t_pic,vid_teacher,subject_name,t_name,sub_start_at,sub_end_at from video v,subjects s,teachers t where vid_type='live' and s.subject_id=vid_sub and vid_teacher=t.t_id and v.enb='1' and vid_class='$classid' order by sub_start_at asc limit 30 ";
	$res = mysqli_query($conn,$sql);
	$arr=array();
	$i=0;
	while($row = mysqli_fetch_array($res)){
...
function getTalks(){
	global $conn;
	$ecode = $_SESSION["eid"];
	$sql="select q_details,student_name,qdate from ask_management a , students s where a.ecode=s.ecode and a.ecode='$ecode' order by ask_id desc limit 20 ";
	$res = mysqli_query($conn,$sql);
	$arr=array();
	$i=0;
	while($row = mysqli_fetch_array($res)){
...
function getNotifications($type){
	global $conn;
	$sql="SELECT * FROM notifications WHERE to_type='$type' AND status='1' order by id desc  LIMIT 10";
	$res=mysqli_query($conn,$sql);
	$i=0; $arr=array();
	while($row=mysqli_fetch_array($res)){
		$arr[$i]['id']=$row['id'];
...
function getClassTeacher(& $ecode){
	global $conn,$filelocation;
...
function getAllStudents(){
	global $conn,$filelocation;
	$class=$_SESSION["class"];
	$sql="select std_id,ecode,roll_no,student_name,image from students where dept_id='$class' and status=1";
	$res=mysqli_query($conn,$sql);
	$i=0;
	while($row=mysqli_fetch_array($res)){
...
function getStudyMaterials(){
	...
	$class=$_SESSION["class"];
	$arr=array();	
	$sql="SELECT COUNT(`subject`) as cnt,subject,subject_name from study_documents d,subjects s WHERE d.subject=s.subject_id and d.class=1 GROUP by subject";
	$res = $conn -> query($sql);
	if($res->num_rows>0){
		$i=0;
		while($row = $res->fetch_assoc()){
...
function checkTeacherOnline(&$teacherid){
	...
	if ($teacher_sql->num_rows > 0){
		while($teacher_row = $teacher_sql->fetch_assoc()){
		  if(isset($isOnline)){
			if($isOnline=='1'){
			  echo $teacher_row['t_name']." <span class='ol_stats'></span>";
			}
			else{
...
function getCourses(&$subject_id,& $emp_ecode){
	...
	if ($get_records->num_rows> 0){
		...
		while($row = $get_records->fetch_assoc()){
			...
			$arr[$i]['cthumb']= 'uploads/images/courses/'.$row['cthumb'];
...
function sel_teachers($selclass=''){
	global $conn;
	$sql= "SELECT t_name,t_id FROM teachers where t_isActive='1'";
	$result = $conn->query($sql);
	if ($result->num_rows > 0)
	{
...
function getStudentsWatchingVideos($video){
	global $conn,$filelocation;
	$sql="SELECT student_name,image FROM session_watching w,students s WHERE student=ecode AND video='$video'";
	$res=mysqli_query($conn,$sql);
	$i=0;$arr=array();
	while($row=mysqli_fetch_array($res)){
	$arr[$i]['student_name']=$row['student_name'];
...
function getAssignment(& $id, & $type){
	...
	if($type=='freetext' || $type=='tbl_freetext'){
		...
		$sql="SELECT f.question,minwords,maxwords,f.id,opendate,closedate,subject_name FROM tbl_freetext f,subjects s WHERE s.subject_id=f.subject AND f.id='$id'";
		$result = $conn->query($sql);
		...
		$arr['minwords'] = $row['minwords'];		 
		$arr['maxwords'] = $row['maxwords'];		 
		//$arr['evid'] = $row['evid'];		 
	}
...
function getLatestChatUsers(){
	global $conn;
	$sql = "SELECT userid,student_name FROM tbl_chat c, students s WHERE c.userid=s.std_id ORDER BY c.created DESC LIMIT 15";
	$res = mysqli_query($conn,$sql);	
	$arr= array();
	$i=0;
	while($row = mysqli_fetch_array($res)){
