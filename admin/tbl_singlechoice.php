  ...
  <div class="form-group">
	<div class="row">
		<div class="col-6">
			...
			<input type="text" class="form-control d-inline-block wd-80" name='singlemarks' id='singlemarks' maxlength=2 size=2  onkeypress="if ( isNaN( String.fromCharCode(event.keyCode) )) return false;" value="<?=$data['marks']?>">
		</div>		
	</div>
  </div>
  <div class="form-group">
...
<div id='singleanswerchoice'>
	...
	<div class="form-group" id='singleans<?=$k?>'>
		<div class='left anspadd'>ans<?=$k?></div> <div class='left'><input type='text' name='singlechoiceans[]' id='singlechoiceans<?=$k?>' value="<?=$options[$i]?>"> </div><div class='left'><input type="radio" id="sinans<?=$k?>" name="singleanswer" value="<?=$k?>" <?php if($k==$data['answer']) echo'checked';?>></div>		
	</div>
	<?php } ?>
</div>
	<div class='form-group'>
