  ...
  <div class="d-flex justify-content-between align-items-center flex-wrap grid-margin">
          <nav class="page-breadcrumb">
            <ol class="breadcrumb">
              ...
              <li class="breadcrumb-item"><a href="index.php?action=exams&class=<?=$_GET['class']?>&subject=<?=$_GET['subject']?>">Exams</a></li>
              <li class="breadcrumb-item active" aria-current="page">Add New Exam</li>
            </ol>
          </nav>          
        </div>
		<input type='hidden' name='evid' id='evid' value="<?php if(isset($_SESSION['evid'])) echo $_SESSION['evid']?>">
                  ...
                  <div class="form-group">
                      <div class="row">
                          <div class="col-6">
                              ...
                              <input type="text" class="form-control" readonly id='tot_marks' name='tot_marks' value="<?php if(isset($_SESSION['tot_marks'])) echo $_SESSION['tot_marks']?>">
                          </div>                          
                      </div>  
                  </div>        
...
<div id='matchrows'>	
  <div class="form-group" id='matchrows'>	
	<div class='rowmatch' id='rowpair1'>
		<div class='matchcol'>Row1 <input type='text' name='matchrowq[]' id='matchrowq1' onkeyup="setQuestion(this.value,1)" value=''></div>
		<div class='matchcol'>Row1 <input type='text' name='matchrowopt[]' id='matchrowopt1' onkeyup="setAnswer(this.value,1)" value=''></div>
	</div>
 </div>	
	<div class="form-group" id='matchrows'>			
	<div class='rowmatch' id='rowpair2'>
		<div class='matchcol'>Row2 <input type='text' name='matchrowq[]' id='matchrowq2' onkeyup="setQuestion(this.value,2)" value=''> </div>
		<div class='matchcol'>Row2 <input type='text' name='matchrowopt[]' id='matchrowopt2' onkeyup="setAnswer(this.value,2)" value=''></div>
	</div>
	</div>
<div class="form-group" id='matchrows'>	
	<div class='rowmatch'  id='rowpair3'>
		<div class='matchcol'>Row3 <input type='text' name='matchrowq[]' id='matchrowq3' onkeyup="setQuestion(this.value,3)" value=''></div>
		<div class='matchcol'>Row3 <input type='text' name='matchrowopt[]' id='matchrowopt3' onkeyup="setAnswer(this.value,3)" value=''></div>
	</div>			
  </div>	
  </div>	
...
<input type='hidden' id='noofsinglechoice' name='noofsinglechoice' value='3'>                     
  ...
  <div class="form-group">
	...
	<div class="form-group" id='singleans2'>
		<div class='left anspadd'>ans2</div> <div class='left'><input type='text' name='singlechoiceans[]' id='singlechoiceans2'> </div><div class='left'><input type="radio" id="sinans2" name="singleanswer" value="2" checked></div>		
	</div>
...
$( "#selectarea" ).change(function() {
 ...
 $( document ).ready(function() {
	...
	if(evid=='') {$('#selectarea').hide();$('#evsave').show();}
		...
		$("#evaluationfrm").on('submit', function(e) {
			e.preventDefault();
			if(evaluationvalid()){
			...
			$.ajax({
				url: 'ajax/operations.php?type=evolution&class='+dclass+'&subject='+sub,
				data: formData,
				type: 'POST',
	...
	$("#frmmatch").on('submit', function(e) {
		...
		var formData = new FormData(this);
		$.ajax({
			url: 'ajax/operations.php?type=matchans&class='+dclass+'&subject='+sub,
			data: formData,
			type: 'POST',
			success: function(data) {
				var obj = JSON.parse(data);
	...
	$("#frmsinglechoice").on('submit', function(e) {
		...
		var formData = new FormData(this);
		$.ajax({
			url: 'ajax/operations.php?type=singlechoice&class='+dclass+'&subject='+sub,
			data: formData,
			type: 'POST',
			success: function(data) {
				var obj = JSON.parse(data);
	...
	$("#frmmultiplechoice").on('submit', function(e) {
		...
		var formData = new FormData(this);
		$.ajax({
			url: 'ajax/operations.php?type=multiplechoice&class='+dclass+'&subject='+sub,
			data: formData,
			type: 'POST',
			success: function(data) {
				var obj = JSON.parse(data);
	...
	$("#frmfreetext").on('submit', function(e) {
		...
		var formData = new FormData(this);
		$.ajax({
			url: 'ajax/operations.php?type=freetext&class='+dclass+'&subject='+sub,
			data: formData,
			type: 'POST',
			success: function(data) {
				var obj = JSON.parse(data);
	...
	$("#frmuploadmedia").on('submit', function(e) {
		...
		var formData = new FormData(this);
		$.ajax({
			url: 'ajax/operations.php?type=uploadimage&class='+dclass+'&subject='+sub,
			data: formData,
			type: 'POST',
			success: function(data) {
				var obj = JSON.parse(data);
