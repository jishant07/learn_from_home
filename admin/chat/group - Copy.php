...
<head>
    ...
    socket.on('chat_message',(chatInfo)=> {
        ...
        if(chatInfo.userName !== userName)
        {
			...
			let time =$('#ctime').val();
			let chtml = `<div class="conversation-list" style='float:left'>
		   ...
		   //$('.chat-list').append("<li class='list-group-item' style='text-align:left'>" + chatInfo.message + " //:" + chatInfo.userName)
		    $('.chat-list').append(chtml);
        }
        else
        {
			let message = chatInfo.message.split('Upic#1*');
			let time =$('#ctime').val();
			let chtml = `<div class="conversation-list" style='float:right'>
                                        <div class="chat-avatar">
                                            <img src="../uploads/images/students/${message[1]}" alt="">
                                        </div>
