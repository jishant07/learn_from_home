  ...
  <div class="form-group">
	<div class="row">
		<div class="col-6">
			...
			<input type="text" class="form-control d-inline-block wd-80" name='freetextmarks' id='freetextmarks' maxlength=2 size=2  onkeypress="if ( isNaN( String.fromCharCode(event.keyCode) )) return false;" value="<?=$data['marks']?>">
		</div>		
	</div>
  </div>
  <div class="form-group">
	<label>Question</label>
	<input type="text" class="form-control" placeholder="Title" name='freetextquestion' id='freetextquestion' value="<?=$data['question']?>">
  </div>
  <div class="form-group">
	<label>Discription</label>
	<textarea class="form-control" placeholder="Discription" rows="5" id='txtanswer' name='txtanswer'><?=stripslashes($data['answer'])?></textarea>
  </div>
...
<script>
 $( document ).ready(function() {
	...
	$("#frmfreetext").on('submit', function(e) {
		...
		$.ajax({
			url: 'ajax/editoperations.php?type=freetext&id='+id,
			data: formData,
			type: 'POST',
