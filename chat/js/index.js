...
$(window).load(function() {
 $messages.mCustomScrollbar();
  ...
  firebase.database().ref("messages").on("child_added", function (snapshot) {
	...
	if(chatwindow==1){		
	if (snapshot.val().sender == myName && snapshot.val().receiver==receiver && snapshot.val().chatwindow==1) {
		//let sdate =setDate()
	...
	if(snapshot.val().receiver == myName && snapshot.val().sendername.trim()==receiver_name){
		removeChatMessage(receiver,myName);
		let gethtml=`<li>
		<div class="conversation-list">
				<div class="user-chat-content" id="message-${snapshot.key}">
					<div class="ctext-wrap">
      ...
      $(gethtml).appendTo($('.mCSB_container')).addClass('new');
		...
		if (snapshot.val().sender == myName && snapshot.val().chatwindow==2) {
			//saveMessage(snapshot);
		//let sdate =setDate()
		let sendhtml=`<li class="right">
			<div class="conversation-list">    
